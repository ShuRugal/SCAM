...
namespace IngameScript
{
    class Program : MyGridProgram
    {
        ...
        public static class Loggify
        {
            ...
            static IMyTextSurface selfLog;
            static List<IMyTextSurface> lcds = new List<IMyTextSurface>();
            ...
            {
                echoClbk = echo;
                selfLog = me.GetSurface(0);
                selfLog.ContentType = ContentType.TEXT_AND_IMAGE;
                selfLog.WriteText("");
            ...
            public static void ClearLog()
            {
                ...
                {   
                    lcd?.WriteText("Log Cleared By User Command");
                    linesToLog.Clear();
                    //Log($"void clearlog cleared external LCD {lcd.DisplayName}");
                    i++;
                }
            }
            ...
            {
                IMyTextSurface lcd = s;
                /* Set some parameters of the LCD panel. */
                lcd.ContentType = ContentType.TEXT_AND_IMAGE;
                //lcd.FontColor = new Color(r: 128, g: 0, b: 128); //stop forcing log text color, let user define this.
                lcd.Font = "Monospace"; // Time stamps will always have the same length.
                lcd.FontSize = 0.65f;
                /* Clear LCD contents. */
                bClearLCD = true;
                lcds.Add(lcd);
                Log($"Added LCD {lcd.Name}, {lcd.DisplayName}, LCD count is {lcds.Count}");
            }
                ...
                if (linesToLog.Any())
                {
                    if (lcds.Count > 1)
                    {
                        foreach (IMyTextSurface lcd in lcds)
                        {
                            if (bClearLCD)
                            {
                                lcd?.WriteText("Log Cleared By Program");
                                bClearLCD = false;
                                //Log($"cleared external LCD {lcd.DisplayName}");
                            }
                            linesToLog.Reverse();
                            var t = string.Join("\n", linesToLog) + "\n" + lcd?.GetText();
                            ...
                            if (t.Length > u)
                                t = t.Substring(0, u - 1);
                            lcd?.WriteText(t);
                            //Log($"attempted to write to {lcd.DisplayName}");
                        }
                        linesToLog.Clear();
                    }
                        ...
                        Log("did not write to external LCD");
        ...
        /** \brief Abbreviation for Loggify.Log(). */
        public static void Log(string msg, Loggify.LogLevel lvl = Loggify.LogLevel.Notice)
        {
            Loggify.Log(msg, lvl);
        }
        ...
        void Ctor()
        {
            ...
            Loggify.Init(Echo, GridTerminalSystem, Me);
            ...
            Toggle.Init(new Dictionary<string, bool>
    {
        ...
        { "cc", true }
            ...
            {
                Loggify.Echo("State load failed, clearing Storage now");
                stateWrapper.Save();
            ...
            else
            {
                ...
                //var selfLog = (IMyTextPanel)GridTerminalSystem.GetBlockWithId(stateWrapper.PState.logLCD);
                //if (selfLog != null)
                    //Loggify.SetLCD(selfLog);
            }
            ...
            this.commandRegistry = new CommandRegistry(
                ...
                },
                {
                    "add-panel", (parts) => {
                        ...
                        if (p != null)
                        {
                            Loggify.Log($"Added {p.CustomName} as GUI panel");
                ...
                },
                {
                    "add-gui-controller", (parts) => {
                        ...
                        Loggify.Log($"Added {guiSeat.CustomName} as GUI controller");
                        var cdata = guiSeat.CustomData; //check custom data to see if user has asked for logs on any panel
                        if (cdata.Contains("SCAMlogs") & (cdata.Contains("0") || cdata.Contains("1") || cdata.Contains("2") || cdata.Contains("3") || cdata.Contains("4"))){
                            int dnum = 0;
                            foreach (string row in cdata.Split('\n')) //read individual rows of custom data to find where display number was defined
                            {
                                if (row.Contains("SCAMlogs"))
                                {
                                    var splitty = row.Split('@');
                                    Int32.TryParse(splitty[1], out dnum);   //extract display number
                                }
                            }
                            var p = (guiSeat as IMyTextSurfaceProvider).GetSurface(dnum); //get the identified surface
                            Loggify.SetLCD(p);  //add it to the list of loggers
                        }
                    }
                },
                {
                    "add-logger", (parts) => {
                        List<IMyTextPanel> tp = new List<IMyTextPanel>();
                        GridTerminalSystem.GetBlocksOfType(tp, x => x.IsSameConstructAs(Me) && x.CustomName.Contains(parts[2]));
                        var p = tp.FirstOrDefault();
                        if (p == null)
                            return; // LCD panel not found.
						Loggify.SetLCD(p);
                        //Loggify.Log("Set logging LCD screen to: " + p.CustomName, Loggify.LogLevel.Notice);
                        stateWrapper.PState.logLCD = p.EntityId;
                    }
                },
                {
                    "set-role", (parts) => Log("command:set-role is deprecated.", Loggify.LogLevel.Warning)
                },
                {
                ...
                {
                    "init-airspace", (parts) => {
                        Log("Warning: Initizing airspace by force. Possible undefined behaviour.", Loggify.LogLevel.Warning);
            ...
            if (stateWrapper.PState.ShaftStates.Count > 0)
            {
                ...
                Loggify.Log($"Restored task from pstate, shaft count: {cap.Count}");
            }
        }
        ...
        public class PersistentState
        {
            ...
            "ShaftStates=" + string.Join(":", ShaftStates),
                ...
                Log("Serialised persistent state: " + s, Loggify.LogLevel.Debug);
                return s;
            }
        ...
        void Main(string param, UpdateType updateType)
        {
            ...
            Loggify.Echo($"Version: {Ver}");
            Loggify.Echo(dispatcherService.ToString());
            ...
            if (Toggle.C.Check("show-pstate"))
                Loggify.Echo(stateWrapper.PState.ToString());
            ...
            Loggify.Echo($"InstructionCount (Max): {Runtime.CurrentInstructionCount} ({iCount})");
            Loggify.Echo($"Processed in {Runtime.LastRunTimeMs:f3} ms");
        }
        ...
        public void GPStaskHandler(string[] cmdString)
        {
            ...
            {
                Loggify.Log("Cannot start new task: All agents must be in Idle or Disabled state.");
                return;
            }
            ...
            else
            {
                if (guiSeat == null)
                {
                    Loggify.Log("WARNING: the normal was not supplied and there is no Control Station available to check if we are in gravity");
                    n = -dispatcherService.dockHost.GetNormal();
                    Loggify.Log("Using 'first dock connector Backward' as a normal");
                }
                else
                {
                    Vector3D pCent;
                    if (guiSeat.TryGetPlanetPosition(out pCent))
                    {
                        ...
                        Loggify.Log("Using mining-center-to-planet-center direction as a normal because we are in gravity");
                    }
                    else
                    {
                        n = -dispatcherService.dockHost.GetNormal();
            ...
            if (!string.IsNullOrEmpty(c))
            {
                ...
                Loggify.Log("To use this mode specify group-constraint value and make sure you have intended circular-pattern-shaft-radius");
        }
            ...
            if (cam != null)
            {
                ...
                if (cam.CanScan(pos))
                {
                    ...
                    if (!dei.IsEmpty())
                    {
                        ...
                        {
                            Loggify.Log("Successfully got mining center and mining normal");
                        ...
                        {
                            Loggify.Log($"RaycastTaskHandler failed to get castedNormal or castedSurfacePoint");
                        }
                    }
                }
                else
                {
                    Loggify.Log($"RaycastTaskHandler couldn't raycast initial position. Camera '{cam.CustomName}' had {cam.AvailableScanRange} AvailableScanRange");
                }
            }
            else
            {
        ...
        public class Dispatcher
        {
            ...
            private void EnqueueLockRequest(long src, string lockName)
            {
                ...
                if (idx < 0)
                {
                    ...
                    Log("Airspace permission request added to requests queue: " + GetSubordinateName(src) + " / " + lockName, Loggify.LogLevel.Debug);
                    return;
                }
            ...
            public void GrantAirspaceLocks()
            {
                ...
                for (int i = 0; i < stateWrapper.PState.airspaceLockRequests.Count(); ++i)
                {
                    ...
                    if (sb == null)
                    {
                        ...
                        Log($"Agent {stateWrapper.PState.airspaceLockRequests[i].id} is no longer a subordinate. Dropping its request for airspace lock.", Loggify.LogLevel.Warning);
                ...
                Log(cand.lockName + " granted to " + GetSubordinateName(cand.id), Loggify.LogLevel.Debug);
            }
            ...
            public void HandleIGC(List<MyIGCMessage> uniMsgs)
            {
                ...
                while (minerChannel.HasPendingMessage)
                {
                    ...
                    {
                        var sectionName = msg.Data.ToString().Split(':')[1];
                        Log("(Dispatcher) received lock-released notification " + sectionName + " from " + GetSubordinateName(msg.Source), Loggify.LogLevel.Debug);
                ...
                while (minerHandshakeChannel.HasPendingMessage)
                {
                    ...
                    {
                        Log("Ignoring handshake broadcast with malformed transponder data.", Loggify.LogLevel.Warning);
                    ...
                    if (data.Item3 != Ver)
                    {
                        Log($"Ignoring handshake broadcast by {data.Item2.Item1.Item2}: Wrong s/w version {data.Item3} (vs {Ver}).", Loggify.LogLevel.Warning);
                        continue;
                    }

                    Log($"Initiated handshake by {data.Item2.Item1.Item2}, group tag: {data.Item1}", Loggify.LogLevel.Notice);
                ...
                foreach (var msg in uniMsgs)
                {
                    ...
                    if (msg.Tag.Contains("request-new"))
                    {
                        if (msg.Tag == "shaft-complete-request-new")
                        {
                            ...
                            Log(GetSubordinateName(msg.Source) + $" reports shaft {msg.Data} complete.", Loggify.LogLevel.Notice);
                        }
                        ...
                        if ((CurrentTask != null) && AssignNewShaft(ref entry, ref getabove, ref shId))
                        {
                            ...
                            Loggify.Log($"Assigned new shaft {shId} to " + GetSubordinateName(msg.Source) + '.', Loggify.LogLevel.Notice);
                        }
                        else
                        {
                ...
                foreach (var s in subordinates)
                {
                    ...
                    Loggify.Echo(s.Report.name + ": F/L=" + s_fl + ", LCK: " + s.ObtainedLock);
                }
            }
            ...
            public void InitializeAirspace()
            {
                ...
                Log("Airspace initialised to current dispatcher location.", Loggify.LogLevel.Notice);
            }
            ...
            public float ReserveFlightLevel(long id)
            {
                ...
                if (ex >= 0)
                {
                    ...
                    Log($"Existing flight level [{fl.h0}, {fl.h1}] for " + GetSubordinateName(fl.agent), Loggify.LogLevel.Debug);
                ...
                Log($"Reserved flight level [{fll.h0}, {fll.h1}] for " + GetSubordinateName(id), Loggify.LogLevel.Debug);
            ...
            {
                Log("CurrentTask.RequestShaft", Loggify.LogLevel.Debug);
        ...
        class Scheduler
        {
            ...
            public void HandleTick()
            {
                if (q.Count > 0)
                {
                    Loggify.Echo("Scheduled actions count:" + q.Count);
                    var c = q.Peek();
        ...
        void CheckExpireNTV()
        {
            ...
            {
                Loggify.Echo(x.Name + ((x.Position.Value == Vector3D.Zero) ? " Zero!" : " OK"));
        ...
        public class DockHost
        {
            ...
            public DockHost(IMyProgrammableBlock me, Dispatcher disp, IMyGridTerminalSystem gts)
            {
                ...
                if (ports.Count == 0)
                {
                    Loggify.Log($"Error: No available docking ports. (Name must contain {DockHostTag}.)", Loggify.LogLevel.Critical);
                    return;
                }
                ...
                {
                    Loggify.Log("Error: Docking ports are not colinear.", Loggify.LogLevel.Critical);
                    ports.Clear();
                    return;
                }
            ...
            public void Handle(IMyIntergridCommunicationSystem i, int t)
            {
                ...
                foreach (var s in dockRequests)
                    Loggify.Echo(s + " awaits docking");
                foreach (var s in depRequests)
                    Loggify.Echo(s + " awaits dep");
                ...
                if (dockRequests.Any())
                {
                    ...
                    if (fd != null)
                    {
                        ...
                        Loggify.Log("Assigned docking port " + fd.CustomName + " to " + dispatcher.GetSubordinateName(id), Loggify.LogLevel.Notice);
                ...
                if (depRequests.Any())
                {
                    foreach (var s in depRequests)
                    {
                        Loggify.Echo(s + " awaits departure");
                    }
                    ...
                    if (bd != null)
                    {
                        ...
                        Loggify.Log($"Sent 0-node departure path");
                ...
                foreach (var d in ports.Where(d => !string.IsNullOrEmpty(d.CustomData)))
                {
                    ...
                    Loggify.Echo($"Channeling DV to {id}");
        ...
        public class GuiHandler
        {
            ...
            )
            {
                ...
                checkBox.bkSprite0 = new MySprite(SpriteType.TEXTURE, "SquareSimple", new Vector2(0, 0), size, Color.DarkRed);
            ...
            )
            {
                ...
                btn.bkSprite0 = new MySprite(SpriteType.TEXTURE, "SquareSimple", new Vector2(0, 0), btnSize, Color.DarkRed);
            ...
            void DrawAirspace(MySpriteDrawFrame frame)
            {
                ...
                foreach (var fl in _stateWrapper.PState.flightLevels)
                {
                    ...
                    frame.Add(new MySprite(
                        ...
                        new Vector2(W, h),
                        (bBright ? Color.Darken(Color.DarkRed, 0.4f) : Color.Darken(Color.DarkRed, 0.5f))));
            ...
            internal void UpdateMiningScheme(Dispatcher.MiningTask obj)
            {
                ...
                foreach (var t in obj.Shafts)
                {
                    ...
                    if (t.State == ShaftState.Planned)
                        mainCol = Color.Darken(Color.DarkBlue, 0.1f);
                    else if (t.State == ShaftState.Complete)
                        mainCol = Color.Darken(Color.LightGreen, 0.4f);
                    else if (t.State == ShaftState.InProgress)
                        mainCol = Color.Lighten(Color.Yellow, 0.2f);
                    else if (t.State == ShaftState.Cancelled)
                        mainCol = Color.Darken(Color.Black, 0.2f);

                    var hoverColor = Color.Cyan;
